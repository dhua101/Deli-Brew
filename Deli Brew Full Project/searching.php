      ...
      <div class="col-xs-4">
        <div class="w3-panel top-margin-content">
          ...
          <?php
              ...
              $sql = "SELECT product_id, product_image, product_name FROM products_tbl WHERE product_name LIKE '%{$searchitem}%' OR product_type LIKE '%{$searchitem}%'";
              $query = mysql_query($sql, $connection);

              $sqlnumber = "SELECT COUNT(*) FROM products_tbl WHERE product_name LIKE '%{$searchitem}%' OR product_type LIKE '%{$searchitem}%'";

              echo "<h3><center>Search Results for " . $searchitem . "</center></h3>";

              echo "<table align='center' style='border:1px solid black'>";
              echo "<tr><th style='border:1px solid black'>Item ID</th><th style='border:1px solid black'>Item Image</th><th style='border:1px solid black'>Item Name</th></tr>";

              while($row1 = mysql_fetch_assoc($query, MYSQL_ASSOC)) {
                  echo "<tr><td style='border:1px solid black'>" . $row1['product_id'] . "</td><td style='border:1px solid black'><img src=" . $row1['product_image'] ." style='width:50%'></td><td style='border:1px solid black'>" . $row1['product_name'] . "</td></tr>";
              }
